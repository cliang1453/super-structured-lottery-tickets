...
class AttentionWrapper(nn.Module):
    ...
    def forward(self, query, key, value, key_padding_mask=None, return_scores=False):
        ...
        logits.data.masked_fill_(key_mask.data.bool(), -float('inf'))
        if self.drop_diagonal:
            ...
            logits.data.masked_fill_(diag_mask.bool(), -float('inf'))
...
class LinearSelfAttn(nn.Module):
    ...
    * o_i = softmax(Wx_i) for x_i in X.
    ...
    def forward(self, x, x_mask):
        ...
        scores.data.masked_fill_(x_mask.data.bool(), -float('inf'))
...
class BilinearFlatSim(nn.Module):
    ...
    * o_i = x_i'Wy for x_i in X.
    ...
    def __init__(self, x_size, y_size, opt={}, prefix='seqatt', dropout=None):
        ...
        self.linear = nn.Linear(y_size, x_size) # [768, 768]
    ...
    def forward(self, x, y, x_mask):
        """
        x = batch * len * h1 [8, 79, 768]
        y = batch * h2 [8, 768]
        x_mask = batch * len [8, 34]
        """
        x = self.dropout(x)
        y = self.dropout(y)
        ...
        if x.shape[1] > x_mask.shape[1]:
            x_extend = torch.ByteTensor(x_mask.shape[0], x.shape[1]-x_mask.shape[1]).fill_(1).cuda(non_blocking=True)
        xWy.data.masked_fill_(torch.cat((x_mask, x_extend), dim=1).data.bool(), -float('inf'))
        return xWy
...
class MultiheadAttentionWrapper(nn.Module):
    ...
    def forward(self, query, key, value, key_padding_mask=None):
        ...
        if self.drop_diagonal:
            ...
            attn_weights.data.masked_fill_(diag_mask.bool(), -float('inf'))
